...
function initLilScript(){
	...
	for(j=0;j<scripts.length;j++){
		...
		if(scripts[j].type.toLowerCase().trim() == type && scripts[j].innerText.trim() != ""){
			let codes = scripts[j].innerText;
			ls.runInTag(codes,scripts[j]);
...
function lilscript(){
  ...
  this.runInTag = function(code,tag=null){
      let script = tag;
      if(script == null){
      ...
      script.type = "";
      script.innerText = "";
      script.src = "data:application/javascript;base64," + btoa(ls.run(code));
